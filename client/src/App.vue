...
async function enterAdmin(tab = "overview") { if (!canAdmin.value) { pendingTab.value = tab; if (authMode.value === "casdoor") { location.href = "/api/parent/casdoor/login"; return; } showLogin.value = true; return; } adminTab.value = tab; view.value = "admin"; await loadAdmin(); }
...
async function login() { try { const r = await api("/parent/login", { method: "POST", child: false, body: { pin: pin.value } }); parentToken.value = r.token; localStorage.setItem("sp_parent_jwt", r.token); showLogin.value = false; pin.value = ""; canAdmin.value = true; sessionStorage.setItem("sp_admin", "1"); await load(); view.value = "admin"; adminTab.value = pendingTab.value || (hasStudent.value ? "overview" : "students"); await loadAdmin(); } catch (e) { fail(e); } }
...
function logout() { parentToken.value = ""; localStorage.removeItem("sp_parent_jwt"); canAdmin.value = false; sessionStorage.removeItem("sp_admin"); view.value = "home"; showLogin.value = false; }
...
onMounted(async () => { await loadAuth(); await load(); if (parentToken.value) { markAdmin(); await loadAdmin().catch(() => {}); view.value = "admin"; } await loadWrong(); });
</script>
