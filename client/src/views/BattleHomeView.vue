...
<template>
  <section class="battle-home">
    ...
    <section class="card arena-hero"><div class="arena-trophy">🏆</div><div><h2>{{ battleRank?.my ? `${battleRank.my.tier_emoji} ${battleRank.my.tier}` : '🥉 青铜' }}</h2><p><b>{{ battleRank?.my?.trophies ?? 0 }}</b> 奖杯 · {{ battleRank?.my?.wins || 0 }} 胜 {{ battleRank?.my?.losses || 0 }} 败</p></div><button class="cta" @click="view='battle'; startBattle()">开始匹配对战</button></section>
    <section class="card rank-board"><h3>段位排行榜</h3><ol class="board-list"><li v-for="(e, i) in battleRank?.entries || []" :key="e.child_id" :class="{ me: e.child_id === currentStudent, top3: i < 3 }"><span class="board-rank">{{ ['🥇','🥈','🥉'][i] || e.rank }}</span><span class="board-avatar">{{ e.avatar || '🐣' }}</span><span class="board-name">{{ e.name }}<small>{{ e.tier_emoji }} {{ e.tier }} · {{ e.wins }}胜{{ e.losses }}败</small></span><b class="board-trophy">🏆 {{ e.trophies }}</b></li><li v-if="!(battleRank?.entries || []).length" class="board-empty">还没有人上榜，来当第一个王者！</li></ol></section>
