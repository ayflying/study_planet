...
<template>
  <section class="pet-page">
    ...
    <section v-if="pet" class="card food-box"><h3>🍖 投喂零食</h3><p v-if="pet.snack_msg" class="snack-tip">{{ pet.snack_msg }}</p><p class="muted">完成任务、对战胜利、学习闯关都有可能获得零食哦！</p><div class="food-grid"><button v-for="f in petFoods" :key="f.id" class="food" :disabled="petFeeding" @click="feedPet(f)"><span class="food-emoji">{{ f.emoji }}</span><b>{{ f.name }}</b><small>🍚 +{{ f.hunger }} 💗 +{{ f.affection }} ✨ +{{ f.exp }}</small><span class="food-count" :class="{ zero: !(pet.food_inventory?.[f.id]) }">{{ pet.food_inventory?.[f.id] || 0 }}</span></button></div></section>
  </section>
</template>
